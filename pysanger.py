...
def abi_to_dict(filename):
    ...
    for i, (pos, conf) in enumerate(
        zip(
            ...
            record.annotations["abif_raw"]["PCON1"],
        )
    ):
        if pos > 4 and pos < len(record.annotations["abif_raw"]["DATA9"]) - 5:
            abi_data["conf"].append(conf)
            abi_data["channel"]["G"].append(
                record.annotations["abif_raw"]["DATA9"][pos]
            )
            abi_data["channel"]["A"].append(
                record.annotations["abif_raw"]["DATA10"][pos]
            )
            abi_data["channel"]["T"].append(
                record.annotations["abif_raw"]["DATA11"][pos]
            )
            abi_data["channel"]["C"].append(
                record.annotations["abif_raw"]["DATA12"][pos]
            )

            abi_data["_channel"]["G"].append(
                record.annotations["abif_raw"]["DATA9"][pos - 5]
            )
            abi_data["_channel"]["G"].append(
                record.annotations["abif_raw"]["DATA9"][pos - 3]
            )
            abi_data["_channel"]["G"].append(
                record.annotations["abif_raw"]["DATA9"][pos]
            )
            ...
            abi_data["_channel"]["A"].append(
                record.annotations["abif_raw"]["DATA10"][pos - 5]
            )
            abi_data["_channel"]["A"].append(
                record.annotations["abif_raw"]["DATA10"][pos - 3]
            )
            abi_data["_channel"]["A"].append(
                record.annotations["abif_raw"]["DATA10"][pos]
            )
            ...
            abi_data["_channel"]["T"].append(
                record.annotations["abif_raw"]["DATA11"][pos - 5]
            )
            abi_data["_channel"]["T"].append(
                record.annotations["abif_raw"]["DATA11"][pos - 3]
            )
            abi_data["_channel"]["T"].append(
                record.annotations["abif_raw"]["DATA11"][pos]
            )
            ...
            abi_data["_channel"]["C"].append(
                record.annotations["abif_raw"]["DATA12"][pos - 5]
            )
            abi_data["_channel"]["C"].append(
                record.annotations["abif_raw"]["DATA12"][pos - 3]
            )
            abi_data["_channel"]["C"].append(
                record.annotations["abif_raw"]["DATA12"][pos]
            )
...
def generate_consensusseq(abidata, conf_thresh=10):
    ...
    return (
        conf_consensus_seq,
        conf_consensus_seq.translate(str.maketrans("ATGCN", "TACGN"))[::-1],
    )
...
def _colorbar(ax, ref, matches=None, char=True, fontsize=8, width=0.96):
    bars = ax.bar(
        [item + 0.5 for item in range(len(ref))],
        [0.9] * (len(ref)),
        width=width,
        edgecolor="#BBBBBB",
        linewidth=0.3,
...
def visualize2(
    ...
    if template_limits is not None:
        ...
        if template_start >= template_end:
            raise ValueError(
                "Invalid template limits specified! Template start point should be less than template end point."
            )
    else:
        template_start = 0
        template_end = len(template)
    ...
    if translation_start > t_start_base:
        remove = (
            translation_start - t_start_base
        )  # need to cut to remove this many bases from the start of the template
    else:
        remove = (
            translation_start - t_start_base
        ) % 3  # need to cut to remove this many bases from the start of the template
    ...
    temp_chars = len(at.rstrip("-")) - at.rstrip("-").count(
        "-"
    )  # number of characters in template minus number of gaps
    ...
    for at_codon, as_codon, codon_pos, curr_codon_num in zip(
        at_codons, asub_codons, template_nt_pos, codon_num
    ):
        for at_codon_nt, as_codon_nt, nt_pos in zip(at_codon, as_codon, codon_pos):
            if at_codon_nt == "-":
                ...
                nt_mismatches.append(
                    f"{nt_pos}({at_codon_nt.upper()}->{as_codon_nt.upper()})"
                )
    ...
    stats = pd.DataFrame(
        {
            ...
            "nt_insertions": [insertions],
            "nt_deletions": [deletions],
            "nt_mismatches": [nt_mismatches],
            "silent_mutations": [silent_mutations],
            "missense_mutations": [missense_mutations],
            "gap_mutations": [frameshift_mutations],
        }
    )
    if plot:
        translation_start = max(
            (translation_start - t_start_base) % 3,
            translation_start + ts - t_start_base,
        )  # make sure translation start stays in right reading frame
        ...
        aa_mismatches = []
        for t_aa, s_aa, t_c, s_c in zip(
            atemplate_aa, asubject_aa, atemplate_codons, asubject_codons
        ):
            if t_aa == s_aa:
                if t_aa == "-" or t_aa == "X":
                    aa_mismatches.append(
                        -1
                    )  # gap or unknown, not real match or mismatch
                elif t_c == s_c:
        ...
        axst.set_ylabel(
            "AA Matches", rotation=0, va="center", ha="right", fontsize="small"
        )
        t4 = time.time()
        ax.bar(
            list(range(length)),
        ...
        axtt.set_ylabel(
            "Template AA", rotation=0, va="center", ha="right", fontsize="small"
        )
        ...
        axt.set_ylabel(
            "Template", rotation=0, va="center", ha="right", fontsize="small"
        )
...
def visualize(
    ...
    translation_start = max(
        (translation_start - t_start_base) % 3, translation_start + ts - t_start_base
    )  # make sure translation start stays in right reading frame
    translation_end = min(len(atemplate), translation_end + ts - t_start_base)
    ...
    atemplate_codons = make_codons(
        atemplate, translation_start=translation_start, translation_end=translation_end
    )
    atemplate_aa = translate(atemplate_codons)
    asubject_codons = make_codons(
        asubject, translation_start=translation_start, translation_end=translation_end
    )
    asubject_aa = translate(asubject_codons)
